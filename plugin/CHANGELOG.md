...
> 本版内容与原本计划的 `1.2.3` 完全相同。`1.2.3` 在发布过程中因一次未完成的 `npm publish`（2FA 未确认，`EOTP`）在服务端留下 staged 占位，导致后续发布报 `E409 Cannot publish over previously staged version "1.2.3"`，而该版本又始终未进入 registry（查询返回 404）。为绕开这一脏状态，版本号顺延为 `1.2.4`；`1.2.3` 从未实际发布。
