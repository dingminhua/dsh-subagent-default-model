  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      if (ctx.uiConversation && ctx.uiConversation.events && typeof ctx.uiConversation.events.register === "function") {
        ...
        // The chat view renders `kind: "context"` nodes via ContextMessageNodeView
        // (content/source/provenance/form), reusing the built-in injection row.
        var chatT = ctx.locale.bind(SUBAGENT_ROW_LOCALE);
        var chatModelDefinition = {
          kind: "chat-subagent-model",
          target: "chat",
          match: function (event) {
            return event.type === "request/header" ? { id: String(event.seq), role: "start" } : null;
          },
          start: function (_context, match) {
            var event = match.event;
            var data = event.data || {};
