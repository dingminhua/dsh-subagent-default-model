...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    // 排查时必须递增它。
    var DSM_CLIENT_BUILD = "2026-09-26.5";
    ...
    function SubagentModelRow(props) {
      ...
      // The serialized form state of the last SAVE this card completed, or null.
      // After a save the describe mirror may keep answering the PREVIOUS document
      // for a while (observed live: the write is verifiably in the profile patch
      // while describe keeps returning the old value). Re-seeding the form from
      // such a mirror would resurrect a route the user just deleted — the
      // reported "删掉一个，保存，还是会出现删掉的那个". So while a save's
      // result is held, the re-seed effect only accepts a mirror that AGREES
      // with what was saved; the convergence poll below keeps refreshing the
      // mirror until it does, and only then hands control back.
      var lastSavedFormRef = React.useRef(null);
      React.useEffect(function () {
        var alive = true;
      ...
      React.useEffect(function () {
        ...
        if (lastSavedFormRef.current !== null) {
          ...
          // not overwrite the just-saved draft (that is the resurrection). Only
          // a mirror that agrees with the save — i.e. the Host's reload has
          // actually propagated — may re-seed again.
          if (JSON.stringify(normalizeDefaultModels(value)) !== lastSavedFormRef.current) return;
      ...
      function save() {
        ...
        Promise.resolve().then(function () {
          ...
          // Hold the saved form: the re-seed effect only accepts a mirror that
          // agrees with this from now on (see the effect for why), and the
          // poll below releases the hold once the mirror converges.
          lastSavedFormRef.current = JSON.stringify(normalizeDefaultModels(acceptedValue));
          ...
          if (mirrorLagged) {
            ...
            // stale document it was saved from.
            var POLL_ATTEMPTS = 20;
            var POLL_DELAY_MS = 500;
            var pollMirror = function (attempt) {
              if (lastSavedFormRef.current === null) return; // already converged (or a newer save took over)
              var scope = props.settingsScope;
              var refreshed = scope && typeof scope.refresh === "function" ? scope.refresh() : Promise.resolve();
              Promise.resolve(refreshed).catch(function () {}).then(function () {
                ...
                if (JSON.stringify(normalizeDefaultModels(stored)) === lastSavedFormRef.current) {
